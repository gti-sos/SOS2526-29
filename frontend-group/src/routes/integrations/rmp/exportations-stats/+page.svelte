<script>
  ...
  // Normaliza paises para cruzar wine-stats con exportations-stats.
  function normCountry(str) {
    const cleaned = String(str ?? "").normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "").toLowerCase()
      .replace(/[.,]/g, "").replace(/\s+/g, " ").trim();
    const aliases = { spain: "spain", espana: "spain", germany: "germany", alemania: "germany" };
    return aliases[cleaned] || cleaned;
  }

  // Carga Highcharts y los modulos necesarios para el diagrama de Venn.
  ...
  // Pinta el Venn con paises solo de vinos, solo de exportaciones y compartidos.
  function renderVenn() {
    if (!vennContainer || !Highcharts) return;
    vennChart?.destroy();

    const BASE = 10;
    const intersectionValue = both > 0
      ? Math.max(1, Math.round((both / Math.max(onlyWine + both, onlyArms + both)) * BASE * 2))
      : 1;

    vennChart = Highcharts.chart(vennContainer, {
      chart: { backgroundColor: "transparent", height: 400 },
      title: {
  ...
  // Descarga ambos datasets, agrega valores por pais y prepara tabla y grafica.
  async function load() {
    loading = true;
    error = "";
    try {
      ...
      for (const w of winesArr) {
        ...
        if (!wineByCountry[key]) {
          wineByCountry[key] = { rawName: w.country, count: 0, priceSum: 0, abvSum: 0, types: {} };
        }
...
<div class="page">
  <div class="hero">
    ...
    <h1>Exportations Stats <span class="accent">(SOS2526-13)</span></h1>
    <p class="hero-desc">
  ...
  <section class="source-grid">
    <a class="source-card" href="https://sos2526-13.onrender.com/api/v2/exportations-stats" target="_blank" rel="noopener">
      <span>API alumno SOS (distinto grupo)</span>
      <strong>SOS2526-13 · exportations-stats</strong>
    </a>
    <a class="source-card" href="https://documenter.getpostman.com/view/52406650/2sBXiomVBY" target="_blank" rel="noopener">
  ...
  {:else}
    ...
    <section>
      <h2 class="section-title">Cruce: países productores de vino vs. exportaciones de armamento</h2>
      <p class="table-note">
...
<style>
  ...
  small { display: block; color: #777; font-size: 0.78rem; }
